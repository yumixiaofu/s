	...
	<div class="{if count($Cart.product_groups) > 1}col-sm-3{else}col-sm-2{/if}">
		<div class="card bg-primary text-white-50" style="background-color: #316eea!important;">
			<div class="card-body">
				...
				<h5 class="mt-0 mb-4 text-white"><i class="mdi mdi-alert-circle-outline mr-3"></i>{$Lang.search} "{$Think.get.keywords}" {$Lang.result}
				</h5>
					...
					<div class="row">
						...
						<div class="col-6 firstbox p-0">
							<ul>
								...
								<li class="{if ($Think.get.fid == $groups.id) || (!$Think.get.fid && $firstIndex==0)}active{/if}">
						...
						<div class="{if count($Cart.product_groups) > 1}col-6{else}col-12{/if} secondbox">
							<ul>
								{foreach $Cart.product_groups as $firstIndex=>$groups}
									{if ($groups.id == $Think.get.fid) || (!$Think.get.fid && $firstIndex==0)}
									<li>
										{foreach $groups.second as $secondIndex=>$second}
										<ul>
											<li class="{if ($Think.get.gid == $second.id) || (!$Think.get.gid && $secondIndex==0)}active{/if}">
												<a {if($second.id==$Think.get.gid)} {/if} href="/cart?fid={$second.gid}&gid={$second.id}" class="d-block">{$second.name}</a>
											</li>
										</ul>
										{/foreach}
									</li>
		...
		<!-- <div class="normalhide">
			<select class="form-control selectpicker mr-2" data-style="btn-default">
				{foreach $Cart.product_groups as $firstIndex=>$groups}
				<option value="{$groups.id}" {if ($Think.get.fid==$groups.id) || (!$Think.get.fid && $firstIndex==0)}selected="" {/if} data-content='<a href="/cart?fid={$groups.id}&gid={$groups.second.0.id}" class="d-block">{$groups.name}</a>'>
					
				</option>
				{/foreach}
			</select>
			{foreach $Cart.product_groups as $firstIndex=>$groups}
				{if ($Think.get.fid==$groups.id) || (!$Think.get.fid && $firstIndex==0)}
				<select class="form-control selectpicker my-2" data-style="btn-default">
						{foreach $groups.second as $secondIndex=>$second}
						<option value="{$second.id}" {if ($Think.get.gid==$second.id) || (!$Think.get.gid && $secondIndex==0)}selected="" {/if} data-content='<a href="/cart?fid={$second.gid}&gid={$second.id}" class="d-block">{$second.name}</a>'>
